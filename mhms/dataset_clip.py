...
class CNNMultimodalDatasetCLIP(Dataset):
    def __init__(self, data_dir="cnn_data", embeddings_dir="embeddings_clip", 
        ...
        if not os.path.exists(text_dir) or not os.path.exists(visual_dir):
            raise ValueError(
                ...
                f"Please run: python embedding_pipeline_clip.py"
            )
        ...
        print(f"CLIP Dataset initialized with {len(self.samples)} valid multimodal cases.")

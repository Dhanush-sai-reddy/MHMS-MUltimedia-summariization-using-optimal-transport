"""
MHMS Embedding Pipeline (Qwen 34B Unified Vision-Language)
===========================================================
Extracts unified multimodal embeddings using Qwen 34B:
...
# Qwen 34B from HuggingFace
from transformers import Qwen3VLForConditionalGeneration, AutoProcessor
from qwen_vl_utils import process_vision_info

# ─── 1. INITIALIZE MODEL ──────────────────────────────────────────────

print("Loading Qwen2.5-34B-Instruct model (~9GB weights, may take a few minutes to load)...")

# Device Configuration
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")

# Model ID - Qwen2.5-34B-Instruct (smallest available, ~9GB)
MODEL_ID = "Qwen/Qwen2.5-34B-Instruct"  # or "Qwen/Qwen3-VL-8B-Instruct" for higher quality
...
# Qwen2.5-34B-Instruct uses 2560 dim (text config) or 2048 (vision config)
try:
...
def extract_text_embeddings_qwen(sentences: List[str], batch_size: int = 8) -> np.ndarray:
    """
    Encode sentences using Qwen 34B text encoder.
    ...
    with torch.no_grad():
        for i in range(0, len(sentences), batch_size):
            ...
            # Format for Qwen 34B: each input is a conversation with text
            conversations = []
            for sent in batch:
...
def extract_visual_embeddings_qwen(image_paths: List[str], batch_size: int = 4) -> np.ndarray:
    """
    Encode images using Qwen 34B vision encoder.
    Extracts hidden states from the last layer as embeddings.
    ...
    with torch.no_grad():
        for i in range(0, len(image_paths), batch_size):
            ...
            # Format for Qwen 34B: each input is a conversation with image
            conversations = []
            valid_paths = []
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"  MHMS Embedding Pipeline (Qwen 34B Unified)")
    ...
    print(f"{'=' * 60}")
    print(f"\nNext steps:")
    print(f"  1. Update dataset.py to use embeddings_qwen34b/")

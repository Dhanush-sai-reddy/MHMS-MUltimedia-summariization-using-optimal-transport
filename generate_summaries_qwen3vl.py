...
def generate_and_save_summaries():
    ...
    if len(dataset) == 0:
        print("Dataset is empty. Ensure embeddings_qwen3vl has matching features.")
        return

    # 2. Initialize the Model framework
    print("Initializing MHMS Qwen3VL Framework...")
    model = MHMS_Qwen3VL(
        embedding_dim=EMBEDDING_DIM,
    ...
    final_path = os.path.join(CHECKPOINT_DIR, "mhms_qwen3vl_final.pth")
    if os.path.exists(final_path):
        # We need to just load the weights, wait, wait... train_qwen3vl.py saves `model.state_dict()` in mhms_qwen3vl_final.pth
        model.load_state_dict(torch.load(final_path, map_location=device))
        print("Pre-trained MHMS Qwen3VL weights loaded successfully!")
    else:
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            # Since MHMS_Qwen3VL doesn't have an OT align inference function written yet,
            # we'll use ot_loss from outputs or do our own fast OT block or match by probs
            # Actually train_qwen3vl computes OT matrix. Here we just take top text nodes

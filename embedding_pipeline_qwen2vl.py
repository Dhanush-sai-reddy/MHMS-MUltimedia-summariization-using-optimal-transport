...
Model: Qwen/Qwen2-VL-8B (or Qwen2-VL-2B for lighter memory)
...
# Load environment variables from .env to capture HF_TOKEN silently
load_dotenv()
...
print("Loading Qwen2-VL-2B-Instruct model (~9GB weights, may take a few minutes to load)...")

# Device Configuration
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")

# Model ID - Qwen2-VL-2B-Instruct (smallest available, ~9GB)
MODEL_ID = "Qwen/Qwen2-VL-2B-Instruct"  # or "Qwen/Qwen2-VL-8B-Instruct" for higher quality
...
# Qwen2-VL-2B-Instruct uses 2560 dim (text config) or 2048 (vision config)
try:
...
def extract_text_embeddings_qwen(sentences: List[str], batch_size: int = 8) -> np.ndarray:
    """
    Encode sentences using Qwen2 VL text encoder.
    ...
    with torch.no_grad():
        for i in range(0, len(sentences), batch_size):
            ...
            # Format for Qwen2 VL: each input is a conversation with text
            conversations = []
            for sent in batch:
...
def extract_visual_embeddings_qwen(image_paths: List[str], batch_size: int = 4) -> np.ndarray:
    """
    Encode images using Qwen2 VL vision encoder.
    Extracts hidden states from the last layer as embeddings.
    ...
    with torch.no_grad():
        for i in range(0, len(image_paths), batch_size):
            ...
            # Format for Qwen2 VL: each input is a conversation with image
            conversations = []
            valid_paths = []
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"  MHMS Embedding Pipeline (Qwen2 VL Unified)")
